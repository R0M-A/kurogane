...
for (let i = 0; i < 1_000; i++) await kurogane.invoke('echo', { i })
}
...
// rtBytes = aggregate round-trip transfer volume
// Throughput is omitted for variable-size JSON payloads.
const SUITE = [
{ id: 'rpc_latency', name: 'RPC Latency', description: '1,000 JSON echo calls', fn: rpcLatency, rtBytes: null },
{ id: 'binary_burst', name: 'Burst transfer', description: '1 KB × 1,000 binary transfers', fn: binaryBurst, rtBytes: 2 * 1_024 * 1_000 },
{ id: 'binary_medium', name: 'Stream payload', description: '1 MB × 100 binary transfers', fn: binaryMedium, rtBytes: 2 * 1_000_000 * 100 },
{ id: 'binary_bulk', name: 'Bulk transport', description: '5 MB × 100 binary transfers', fn: binaryBulk, rtBytes: 2 * 5_000_000 * 100 },
{ id: 'binary_heavy', name: 'High-pressure transport', description: '10 MB × 50 binary transfers', fn: binaryHeavy, rtBytes: 2 * 10_000_000 * 50 },
{ id: 'binary_extreme', name: 'Sustained saturation', description: '20 MB × 100 binary transfers', fn: binaryExtreme, rtBytes: 2 * 20_000_000 * 100 },
]
...
log(line) {
    console.log(line)
    // createTextNode avoids O(n) string re-allocation from repeated textContent concatenation
